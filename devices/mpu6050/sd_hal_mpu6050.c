...
MPU6050 mpu1;
MPU6050 mpu1Filtered;
char cmdBuf;
TaskHandle_t mainThreadHandle;

static void MPU6050_msgsrv_hook(void* arg)
{
	cmdBuf = ((char*)arg)[0];
	xTaskNotifyGive(mainThreadHandle);
}

static void MPU6050_PrintStats()
{
	static char sbuf[64];
...
MPU6050_Result MPU6050_Init(I2C_HandleTypeDef* I2Cx,MPU6050* DataStruct, MPU6050_Device DeviceNumber, MPU6050_Accelerometer AccelerometerSensitivity, MPU6050_Gyroscope GyroscopeSensitivity)
{
	...
	MSGSRV_register_client(MSGSRV_MPU6050_CLNT, MPU6050_msgsrv_hook);
	...
	MPU6050_SetDataRate(I2Cx,DataStruct, MPU6050_DataRate_100Hz);
...
MPU6050_Result MPU6050_ReadAll(I2C_HandleTypeDef* I2Cx,MPU6050* DataStruct)
{
	...
	if(HAL_I2C_Master_Receive(Handle, (uint16_t)address, data, 14, 1000) != HAL_OK){
		return MPU6050_Result_Error;
	}
	...
	DataStruct->Gyroscope_X = (int16_t)(data[8] << 8 | data[9]) * MPU6050_NORM_FACTOR;
	DataStruct->Gyroscope_Y = (int16_t)(data[10] << 8 | data[11]) * MPU6050_NORM_FACTOR;
	DataStruct->Gyroscope_Z = (int16_t)(data[12] << 8 | data[13]) * MPU6050_NORM_FACTOR;
...
MPU6050_Result MPU6050_FilterCalman(MPU6050* mpuFiltered, MPU6050* mpuCurrent, float coefKalman)
{
		
	mpuFiltered->Accelerometer_X = coefKalman * mpuCurrent->Accelerometer_X + (1 - coefKalman) * mpuFiltered->Accelerometer_X;
	mpuFiltered->Accelerometer_Y = coefKalman * mpuCurrent->Accelerometer_Y + (1 - coefKalman) * mpuFiltered->Accelerometer_Y;
	mpuFiltered->Accelerometer_Z = coefKalman * mpuCurrent->Accelerometer_Z + (1 - coefKalman) * mpuFiltered->Accelerometer_Z;
	mpuFiltered->Gyroscope_X = coefKalman * mpuCurrent->Gyroscope_X + (1 - coefKalman) * mpuFiltered->Gyroscope_X;
	mpuFiltered->Gyroscope_Y = coefKalman * mpuCurrent->Gyroscope_Y + (1 - coefKalman) * mpuFiltered->Gyroscope_Y;
	mpuFiltered->Gyroscope_Z = coefKalman * mpuCurrent->Gyroscope_Z + (1 - coefKalman) * mpuFiltered->Gyroscope_Z;
	
	return MPU6050_Result_Ok;
}


void MPU6050_poll_thread(void* args)
{
	
	while(1)
	{
		//poll mpu6050 controller		
		if(MPU6050_ReadAll(&hi2c2, &mpu1) != MPU6050_Result_Ok){
			taskYIELD();
			MPU6050_Init(&hi2c2,&mpu1,MPU6050_Device_0,MPU6050_Accelerometer_2G,MPU6050_Gyroscope_250s );
			continue;
		}
		
		MPU6050_FilterCalman(&mpu1Filtered, &mpu1, 1);		
		vTaskDelay(500);
	}

}
void MPU6050_main_thread(void* args)
{
	mainThreadHandle = xTaskGetCurrentTaskHandle();
	
	//Init MP6050 controller
	MPU6050_Init(&hi2c2,&mpu1,MPU6050_Device_0,MPU6050_Accelerometer_2G,MPU6050_Gyroscope_250s );
	MPU6050_ReadAll(&hi2c2, &mpu1);
	mpu1Filtered = mpu1;
